...
UFileMput::~UFileMput() {
    ...
    if (m_file_stream && m_file_stream->is_open()) m_file_stream->close();
}
...
int UFileMput::SetUploadFile(const std::string &filepath) {
    ...
    //关闭之前的文件流
    if (!m_file_stream) m_file_stream = new std::ifstream();
    if (m_file_stream && m_file_stream->is_open()) m_file_stream->close();
...
int UFileMput::MUpload(ssize_t blk_idx) {
    ...
    if (blk_idx != -1) m_blk_idx = blk_idx;
    ...
    UCloudHTTPReadParam  rp = {f: NULL, is: *m_is, fsize: bsize, need_total_n: bsize};
    UCloudHTTPWriteParam wp = {f: NULL, os: &data_stream};
...
int UFileMput::ParseMuploadResult(const std::string &body, const std::string &header) {
...
int UFileMput::ParseMFinishResult(const std::string &body, const std::string &header) {
...
std::string UFileMput::MUploadURL() {
    ...
    oss << UFileHost(m_bucket) + EasyPathEscape(m_key) + "?uploadId=" + EasyQueryEscape(m_uploadid);
...
int UFileMput::ETagStream(const std::map<size_t, std::string> &etag_list, std::stringstream &sss) {
