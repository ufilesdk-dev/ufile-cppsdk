...
UFileDownload::UFileDownload() : m_bucket(""), m_key(""), m_os(NULL) {}

UFileDownload::~UFileDownload() {}

int UFileDownload::Download(const std::string &bucket, const std::string &key,
                            std::ostream *os,
...
std::string UFileDownload::DownloadURL(const std::string &bucket,
                                       const std::string &key,
                                       const size_t expires) {
  ...
  char expires_str[32];
  size_t n = snprintf(expires_str, sizeof(expires_str) - 1, "%zd",
                      expires + time(NULL));
  if (expires != 0) {
    m_http->SetQuery("Expires", std::string(expires_str, n));
  }

  ret = digestor.SignWithRequest(m_http, QUERY_STRING_CHECK, bucket, key, "",
                                 signature);
  if (ret) {
    return "";
  }
