#ifndef _UFILESDK_CPP_UCLOUD_HTTP_MIMETYPE_
...
public:
    ...
    std::string InduceType(const std::string &filename, const void *data, const size_t &length) {
        ...
        std::string ext = std::string(pos);
        std::map<std::string, std::string>::iterator it = m_mimetypes.find(ext);
        if (it == m_mimetypes.end()) return "application/octet-stream";
        return it->second;
    }
