...
void HTTPRequest::SetRange(const std::pair<ssize_t, ssize_t> &range) {
  ...
  if (m_curl) {
    m_req_headers =
        curl_slist_append(m_req_headers, ("Range: " + oss.str()).c_str());
  ...
  for (; header; header = header->next) {
    ...
    if (pos)
      headers.insert(std::make_pair<std::string, std::string>(
          std::string(data, pos), std::string(pos + 1)));
  }
  return 0;
}
...
int HTTPRequest::ResponseCode(long *code) {
  ...
  if (curl_easy_getinfo(m_curl, CURLINFO_RESPONSE_CODE, code) != CURLE_OK)
    return -1;
  return 0;
}
